        ...
        for tup in csv.iterrows():
           ...
           #nova sentenca
            if anterior > row[2]:
                ...
                tags = []
            #linha em branco
            if isinstance(row[11], float) and math.isnan(row[11]):
                pass
            else:
                tokens.append(Token(str(row[3])))
                tag = row[11]
                if tag[0] == '(':
                    tag = tag.replace('(','B_').replace('*)', '')
                else:
                    tag = tag.replace('*)', 'O').replace('*','O')
                tags.append(tag)
                verb_idx = 0
                if row[4] == "VERB":
                    verb_idx = 1
                verb_indicator.append(verb_idx)
                anterior = row[2]
